...
private:
    ...
    //Computes (a2 + b2)1=2 without destructive underflow or overflow.
    {
        ...
        if (absa > absb)
        {
            return (T)(absa * sqrt(1.0 + pow(absb / absa, 2)));
        }
        else
        {
            return (T)(absb == 0.0 ? 0.0 : absb * sqrt(1.0 + pow(absa / absb, 2)));
        }
    }
...
public:
    ...
    {
        std::cout << "LTensor:: SolvebyDGESV()\n";

        int N = (*this).get_dim1();
        int M = (*this).get_dim1();
        Marray<T, 1> solution(N);

#ifdef _LTENSOR_HAVE_CLAPACK

        if (M != N)
        {
            std::cerr << "LTensor:: SolvebyDGESV() - N != M.\n";
            return solution;
        }
        ...
        // dgesv_( &N, &nrhs, a, &lda, ipiv, bptr, &ldb, &info );
    ...
    // err returns true if process fails;
    {
        ...
        for (int row = 0; row < N; row++)
        {
            ...
            {
                if (absTrait<T>::function(A(row, col)) > scale(row))
                {
                    scale(row) = absTrait<T>::function(A(row, col));
                }
            }
        }
